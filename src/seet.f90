subroutine seet_main_loop(Fk,Sk,dm_r,sig_sol_loc_ndc,sigma_inf_loc,E1_old,nts)
  use seet
  use gf2, only : nk,nao,iwmax,beta,omegas,ncell,ncellgf2,mu,nuniform,nleg,Sigma_tau
  implicit none
  integer nts
  ...
  integer iv,jv,kv,lv,ic,jc,kc,lc,ii,jj,kk,ll
...
!     call prepare_density_matrix_basis(Skk,xmatk,dm_eig_k,dm_r)
     ...
     call pbc_nao(Skk,dble(dm_r),wannier,xmatk)
     deallocate(dm_eig_k)
  end if
        ...
        do i=1,num_imp
           ...
           do ii=1,norb
              do jj=1,norb  
                 do kk=1,norb
                    do ll=1,norb
                       write(*,*)ii,jj,kk,ll,vint(ii,jj,kk,ll)
                    end do
                 end do
              end do
           end do
           ...
           call eval_hybrid(Fkk,Skk,siglk,sig_sol_loc_ndc_old,sigma_inf_loc_old,Grcsw,sigmaw,deltarw,Sr,Fr,norb,sigma_inf_gf2_imp,vint,i,iter_seet)
           ...
           call system("/export/apps/Python/2.7.2-CentOS6.0/bin/python /export/zgid/zgid/seet_loop_flux/fit_bath_pyf/hybrid.py > fitting_bath.out")
           ...
           call system("/export/apps/Python/2.7.2-CentOS6.0/bin/python /export/zgid/zgid/seet_loop_flux/solvers/solver_2.py > calling_solver.out")
...
!           call calc_sigma_inf_imp(G_sol,norb,sigma_inf_imp,vint,nts)  
           ...
           do m=1,norb   
              do n=1,norb 
                 sigma_inf_imp(m,n)=dble(sigma_imp(m,n,iwmax))
              end do
           end do
...
!           do m=1,norb
!              write(*,*)"imp_orb_numbers",m,imp_orb_numbers(i,m) 
!           end do
!           write(*,*)"tot_sigma_out of ci"
!           do w=1,iwmax
!              if (w<501)write(*,*) w, dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
!              write(5000,*) dimag(omegas(w)), dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
!           end do
 ...
 !           write(*,*)"GF2_loc_self_energy in no in tau"
 !           call flush(6)
 !           do w=1,100
 !              write(*,*)w, dble(sigmagf2t(1,1,w)),dimag(sigmagf2t(1,1,w))
 !           end do
        ...
        write(*,*)"after transform_t_to_w_sigmaGF2_loc0" 
        call flush(6)
!           do w=1,iwmax
!              if (w<501) write(*,*) w, dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
!              write(5003,*) dimag(omegas(w)), dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
!           end do

!           do w=1,iwmax
!              if (w<501) write(*,*) w, dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
!              write(7000,*) dimag(omegas(w)), dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w)),dble(sigmagf2loc_w(1,1,w)),dimag(sigmagf2loc_w(1,1,w))
!              write(7001,*) dimag(omegas(w)), dble(sig_sol_loc_ndc(1,2,w)),dimag(sig_sol_loc_ndc(1,2,w)),dble(sigmagf2loc_w(1,2,w)),dimag(sigmagf2loc_w(1,2,w))
!           end do
        ...
        do w=1,iwmax
           do i=1,nao
              do j=1,nao
                 sig_sol_loc_ndc(i,j,w)=sig_sol_loc_ndc(i,j,w)-sigmagf2loc_w(i,j,w)
              end do
           end do
        end do
               write(*,*)"after transform_t_to_w_sigmaGF2_loc1" 
        call flush(6)
           do w=1,iwmax
!              if (w<501) write(*,*) w, dble(sig_sol_loc_ndc(1,1,w)),dimag(sig_sol_loc_ndc(1,1,w))
              do i=1,nao
                 do j=1,nao
                    write(6000,*) w,i,j,dble(sig_sol_loc_ndc(i,j,w)),dimag(sig_sol_loc_ndc(i,j,w))
                 end do
              end do
           end do
           ...
           do i=1,nao
              do j=1,nao
                 write(7000,*)i,j,dble(sigma_inf_loc(i,j)),dimag(sigma_inf_loc(i,j))
              end do
           end do
           
        !transform sigma to the orginal basis
        write(*,*)"before transform sigma0",sigma_inf_loc(1,1),sigma_inf_loc(1,2),sigma_inf_loc(2,2)


           if(iter_seet>1) then
              sig_sol_loc_ndc=(1-damps)*sig_sol_loc_ndc_old+damps*sig_sol_loc_ndc
           end if
        sig_sol_loc_ndc_old=sig_sol_loc_ndc
         if(iter_seet>1) then
            sigma_inf_loc=(1-dampsinf)*sigma_inf_loc_old+dampsinf*sigma_inf_loc
         end if
        sigma_inf_loc_old=sigma_inf_loc

        write(*,*)"before transform sigma",sigma_inf_loc_old(1,1),sigma_inf_loc_old(1,2),sigma_inf_loc_old(2,2)
        call flush(6)
...
!        do i=1,nao
!           do j=1,nao
!              write(*,*)"SS",i,j,sigma_inf_loc(i,j)
!              do w=1,500
!                 write(*,*)"SS1",w,i,j,sig_sol_loc_ndc(i,j,w)
!              end do
!           end do
!        end do
        nodm=.false.
...
!        write(*,*)"after transform sigma",xmatk(:,:,1),xmatk(:,:,5)
        
        call flush(6)

        dm_r(:,:,:)=dcmplx(0.0d0,0.0d0)
        call mu_dens(Fk,Sk,xmatk,dm_r,sig_sol_loc_ndc,sigma_inf_loc,nts,print_sigl,nodm)
        ...
        call calc_energy_freq_depend_old(mu,Fk,Sk,xmatk,sig_sol_loc_ndc,sigma_inf_loc,E_corr,nts,nuniform,iter_seet)
        ...
        if (fock_reeval)then 
           ...
           Fkk(:,:,:)=Fk(:,:,:)
           Skk(:,:,:)=Sk(:,:,:)
           sigma_inf_loc_old=dcmplx(0.0d0,0.0d0)
           sigma_inf_loc=dcmplx(0.0d0,0.0d0)
        !
        !else
        !   E_S0=E1_old
        ...
        write(*,*) ' energy diff:', energy_seet-energy_seet_old,energy_seet,energy_seet_old
...
subroutine transform_sigma(sig_sol_loc_ndc,sigma_inf_loc,xmatk)
  ...
  do i=1,nao
     do j=1,nao
        write(*,*)"ch loc",ic,i,j,sigma_inf_loc(i,j)
     end do
  end do
...
subroutine eval_G0_inv(Fr,Sr,sigma_inf,deltarw,G0_inv,norb,imp)
  ...
  implicit none
  integer norb,imp
  complex*16, dimension(nao,nao)::Sr,Fr
  complex*16, dimension(norb,norb)::sigma_inf
  complex*16, dimension(norb,norb,iwmax)::deltarw,G0_inv         
  double precision,dimension(:,:),allocatable::f_fit
  integer i,j,k,l,p,q,ic,w,ns,nbath
  complex*16 muomega,delta
  double precision tmp

  nbath=0
  do i=1,norb
     nbath=nbath+imp_bath_numbers(imp,i)
  end do
  ns=norb+nbath
  allocate(f_fit(ns,ns))
  
  !read in the fock matrix 
    ...
    do i=1,ns
       do j=1,i
          ...
          f_fit(i,j)=tmp
          f_fit(j,i)=tmp
          write(*,*)"f_fit",i,j,f_fit(i,j)
       end do
    end do
    close(2020)
  !evaluate delta_fit when needed
  ...
  do w=1,iwmax
     ...
     do p=1,norb
        do q=1,norb
           ...
           delta=dcmplx(0.0d0,0.0d0)
           do k=1,nbath
              delta=delta+(f_fit(p,norb+k)*f_fit(q,norb+k))/(omegas(w)- f_fit(norb+k,norb+k))
           end do

           G0_inv(p,q,w)=muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-delta
           !G0_inv(p,q,w)=muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-deltarw(p,q,w)
!           G0_inv(p,q,w)=muomega*Sr(i,j)-(Fr(i,j))-deltarw(p,q,w)
           if (w<5000) then
              if (p==1 .and. q==1) then
                 write(1515,*)dimag(omegas(w)),dble(muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-deltarw(p,q,w)),dimag(muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-deltarw(p,q,w))
                 write(1516,*)dimag(omegas(w)),dble(muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-delta),dimag(muomega*Sr(i,j)-(Fr(i,j)-sigma_inf(p,q))-delta)
              end if
           end if
        end do
     end do
...
subroutine eval_dyson_impurity(G0_inv,sigma_imp,G_sol,norb)  
   ...
   do w=1,iwmax
      ...
      write(5555,*) dimag(omegas(w)), dble(sigma_imp(1,1,w)),dimag(sigma_imp(1,1,w))
      write(5515,*) dimag(omegas(w)), dble(sigma_imp(1,2,w)),dimag(sigma_imp(1,2,w))
      write(5511,*) dimag(omegas(w)), dble(sigma_imp(2,2,w)),dimag(sigma_imp(2,2,w))


      G_sub_inv(:,:)=G0_inv(:,:,w)
      call invert_complex_matrix(norb,G_sub_inv)   
      write(5005,*) dimag(omegas(w)),dble(G_sub_inv(1,1)),dimag(G_sub_inv(1,1))
      write(5015,*) dimag(omegas(w)),dble(G_sub_inv(1,2)),dimag(G_sub_inv(1,2))
      write(5105,*) dimag(omegas(w)),dble(G_sub_inv(2,1)),dimag(G_sub_inv(2,1))
      write(5115,*) dimag(omegas(w)),dble(G_sub_inv(2,2)),dimag(G_sub_inv(2,2))
  ...
  do w=1,iwmax
     do i=1,norb
        do j=1,norb
           ...
           if (ii==0 .and. jj==0) write(5004,*)dimag(omegas(ww+1)),dble(G_sol(ii+1,jj+1,ww+1)), dimag(G_sol(ii+1,jj+1,ww+1))
           if (ii==0 .and. jj==1) write(5014,*)dimag(omegas(ww+1)),dble(G_sol(ii+1,jj+1,ww+1)), dimag(G_sol(ii+1,jj+1,ww+1))
           if (ii==1 .and. jj==1) write(5114,*)dimag(omegas(ww+1)),dble(G_sol(ii+1,jj+1,ww+1)), dimag(G_sol(ii+1,jj+1,ww+1))
           if (ii==1 .and. jj==0) write(5104,*)dimag(omegas(ww+1)),dble(G_sol(ii+1,jj+1,ww+1)), dimag(G_sol(ii+1,jj+1,ww+1))
        end do
     end do
  end do
  close(100)
...
subroutine prepare_quantities_for_seet(Fk,Sk,siglk,xmatk,sig_sol_loc_ndc,sigma_inf_loc,Grcsw,sigmaw,Gtloc,Sr,Fr,nts,it_seet)
  ...
  integer w,ic,i,j,t,l,k,p,q,unum
  double precision, save::pi=4.D0*DATAN(1.D0)
  double precision taun

 

  if (it_seet==1 .or. fock_reeval==.TRUE.) then
  ...
  unum=950
  open(unum,file='GF_iw.txt',status='replace',action='write',form='formatted')
  ...
  ic=cells_seet
  do w =1,iwmax
     call calc_gwr_inner(w,mu,Grcs,Fk,Sk,sig_sol_loc_ndc(:,:,w),sigma_inf_loc,G1,G2,siglk,cells_seet,unum)
     Grcsw(:,:,w)=Grcs(:,:,ic)
  end do
  ...
  if (it_seet==1 .or. fock_reeval==.TRUE.) then
     ...
     write(*,*)"Fock and overlap"
     do p=1,nao
        do q=1,nao
           write(*,*)"FS",p,q,Fr(p,q),Sr(p,q)
        end do
     end do
...
!           if (k<=1 .and. w==1) then
!              do i=0,nleg,2
!                 write(*,*)i,k,"slk d",siglk(1,1,i,k)
!              end do

!              do i=0,nleg,2
!                 write(*,*)i,k,"slk od",siglk(1,2,i,k)
!              end do
!           end if
        end do
        sigmaw(:,:,w)=dcmplx(0.0d0,0.0d0)
        call FT_k_to_r_ic(Sk(:,:,:), sigmaw(:,:,w),ic)     
!        write(1000,*)dimag(omegas(w)),dble(sigmaw(1,1,w)),dimag(sigmaw(1,1,w))
!        write(1010,*)dimag(omegas(w)),dble(sigmaw(1,2,w)),dimag(sigmaw(1,2,w))
!        write(1011,*)dimag(omegas(w)),dble(sigmaw(2,2,w)),dimag(sigmaw(2,2,w))
!        write(1001,*)dimag(omegas(w)),dble(sigmaw(2,1,w)),dimag(sigmaw(2,1,w))
     end do
     call flush(6)
...
subroutine eval_hybrid(Fk,Sk,siglk,sig_sol_loc_ndc,sigma_inf_loc,Grcsw,sigmaw,deltarw,Sr,Fr,norb,sigma_inf_gf2_imp,vint,imp,iter_seet)
  use seet
  use gf2,only:iwmax,omegas,mu,nao,nleg,nk
  implicit none
  integer norb,imp
  complex*16, dimension(nao,nao,2*nk) :: Sk,Fk
  complex*16, dimension(nao,nao,0:nleg,2*nk) ::siglk
  complex*16, dimension(nao,nao,iwmax)::sig_sol_loc_ndc,Grcsw,sigmaw
  complex*16, dimension(nao,nao)::Sr,Fr,sigma_inf_loc
  ...
  double precision,dimension(norb,norb,norb,norb)::vint
  complex*16, dimension(:,:),allocatable::G_sub_inv
  complex*16, dimension(:,:,:),allocatable::Grcsw_inv
  ...
  allocate(G_sub_inv(norb,norb))
  allocate(Grcsw_inv(norb,norb,cells_seet))
...
!  if(iter_seet == 1 .or. fock_reeval==.TRUE.) then
     call calc_sigma_inf_imp(deltarw,norb,G_sub_inv,vint)
     do p=1,norb 
        do q=1,norb 
           i=imp_orb_numbers(imp,p) 
           j=imp_orb_numbers(imp,q) 
           sigma_inf_gf2_imp(p,q)=G_sub_inv(p,q)
!           write(*,*)"sig_inf_gf2",p,q,sigma_inf_gf2_imp(p,q)
        end do
     end do
!  end if
  call flush(6)
  ...
  do w=1,iwmax
     ...
     do p=1,norb
        do q=1,norb
           i=imp_orb_numbers(imp,p)
           j=imp_orb_numbers(imp,q)
!           Grcsw_inv=dcmplx(0.0d0,0.0d0)
           !call calc_gwr_inv_inner(w,mu,Grcsw_inv,Fk,Sk,sig_sol_loc_ndc(:,:,w),sigma_inf_loc,siglk,cells_seet)
           !G_sub_inv(p,q)=Grcsw_inv(i,j,ic)
           G_sub_inv(p,q)=Grcsw(i,j,w)
           if (p==1 .and. q==1) write(5011,*) dimag(omegas(w)),dble(Grcsw(i,j,w)),dimag(Grcsw(i,j,w))
           if (p==1 .and. q==2) write(5012,*) dimag(omegas(w)),dble(Grcsw(i,j,w)),dimag(Grcsw(i,j,w))
!           write(100,*)p-1,q-1,w-1,dble(Grcsw(i,j,w)),dimag(Grcsw(i,j,w))
        end do
     end do
     call invert_complex_matrix(norb,G_sub_inv)
     do p=1,norb
        do q=1,norb
           ...
           if (p==1 .and. q==1) write(1022,*)dimag(omegas(w)),dble(deltarw(p,q,w)),dimag(deltarw(p,q,w))
           if (p==1 .and. q==2) write(1023,*)dimag(omegas(w)),dble(deltarw(p,q,w)),dimag(deltarw(p,q,w))
        end do
     end do
  end do
  ...
  write(70,"(I3)") norb
     ...
     do p=1,norb
        do q=1,norb
           i=imp_orb_numbers(imp,p)
           j=imp_orb_numbers(imp,q)
           write(20,*)i,j,dble(Fr(i,j)-sigma_inf_gf2_imp(i,j))!-sigma_inf_loc(i,j))
           write(60,*)i,j,dble(sigma_inf_gf2_imp(i,j))!+sigma_inf_loc(i,j))
           do w=1,iwmax
              if(w < 5001) then
           ...
           do r=1,norb
              do s=1,norb
                 write(70,"(F12.8)") dble(vint(p,q,r,s))
              end do
           end do
!           do t=1,nts
...
subroutine read_in_info_for_seet
  ...
  allocate(imp_bath_numbers(num_imp,max_imp_orb))
  imp_orb_numbers=0
  do i=1,num_imp   
     do j=1,num_orb_in_imps(i)
        call fetch_i_bath(imp_orb_numbers(i,j), imp_bath_numbers(i,j),         'imp_orb_numbers         ',ip)
     end do
  end do
  ...
  call fetch_r(damps ,    'damps   ',ip)
  call fetch_r(dampsinf ,    'dampsinf   ',ip)
